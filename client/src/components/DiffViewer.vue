...
function countChanges(singleDiff: string): {
  additions: number
  deletions: number
} {
  let additions = 0
...
<template>
  <div class="space-y-1">
    <!-- Stats summary -->
    <div v-if="stats" class="text-xs text-zinc-500 font-mono whitespace-pre">
      {{ stats }}
    </div>
    ...
    <div
      v-if="uncommitted"
      class="flex items-center justify-between gap-3 px-3 py-2 rounded border border-amber-700/50 bg-amber-950/30 text-sm">
      <span class="text-amber-300"
        >These changes are uncommitted in the worktree.</span
      >
      <button
    ...
    <!-- Loading -->
    <div v-if="loading" class="text-sm text-zinc-500 py-4 text-center">
      Loading diff...
    </div>

    <!-- Error -->
    <div v-else-if="error" class="text-sm text-red-400 py-4 text-center">
      {{ error }}
    </div>

    <!-- No changes -->
    <div v-else-if="!diffText" class="text-sm text-zinc-500 py-4 text-center">
      No changes
    </div>

    <!-- Tabbed diff display -->
    <div
      v-else-if="fileDiffs.length"
      class="rounded border border-zinc-800 overflow-hidden">
      <!-- Tab bar -->
      <div
      ...
      <div
        class="px-2 py-1 bg-[#161b22] text-xs text-zinc-500 font-mono border-b border-zinc-800 truncate">
