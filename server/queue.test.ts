...
function makeDeps() {
  return {
    ...
    updateTask: vi.fn<(id: string, updates: Record<string, unknown>) => Task | undefined>(),
...
describe('TaskQueue', () => {
  ...
  describe('isDependencySatisfied', () => {
    ...
    it('returns true when dependency is approved', () => {
      ...
      deps.getTaskById.mockReturnValue(makeTask({ id: 'dep-1', status: 'approved' }));
      expect(queue.isDependencySatisfied(task)).toBe(true);
    });
    ...
    it('returns false when dependency is in_progress', () => {
      const task = makeTask({ depends_on: 'dep-1' });
      deps.getTaskById.mockReturnValue(makeTask({ id: 'dep-1', status: 'in_progress' }));
      expect(queue.isDependencySatisfied(task)).toBe(false);
    });

    it('returns false when dependency is rejected', () => {
      const task = makeTask({ depends_on: 'dep-1' });
      deps.getTaskById.mockReturnValue(makeTask({ id: 'dep-1', status: 'rejected' }));
      expect(queue.isDependencySatisfied(task)).toBe(false);
    });
  ...
  describe('recomputePositions', () => {
    ...
    it('puts blocked tasks after ready ones', () => {
      ...
      expect(deps.updateTask).toHaveBeenCalledWith('ready', { queue_position: 1 });
      expect(deps.updateTask).toHaveBeenCalledWith('blocked', { queue_position: 2 });
    });
  });
  ...
  describe('dispatch', () => {
    it('transitions queued task to in_progress', () => {
      ...
      expect(deps.updateTask).toHaveBeenCalledWith('t1', { status: 'in_progress' });
      expect(deps.createTaskEvent).toHaveBeenCalledWith('t1', 'dispatched', null);
