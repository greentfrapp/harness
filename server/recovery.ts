...
function recoverTask(deps: RecoveryDeps, task: Task): void {
  ...
  if (!project) {
    ...
    deps.createTaskEvent(task.id, 'recovered', JSON.stringify({
      action: 'error',
  ...
  if (task.worktree_path && task.branch_name) {
    ...
    if (worktreeExists && git.hasCommits(project.repo_path, project.target_branch, task.branch_name)) {
      // Has partial work — push to inbox as error so user can review
      deps.updateTask(task.id, {
        status: 'error',
        error_message: 'Server restarted during execution. Partial work available for review.',
        diff_summary: git.getDiffStats(project.repo_path, project.target_branch, task.branch_name),
      });
      deps.createTaskEvent(task.id, 'recovered', JSON.stringify({
        action: 'error_with_work',
        reason: 'server_restart',
      }));
      console.log(`Recovery: task ${task.id.slice(0, 8)} → inbox (partial work)`);
      ...
      console.log(`Recovery: task ${task.id.slice(0, 8)} → re-queued (no commits)`);
    }
  } else {
...
function reconcileOrphanedWorktrees(deps: RecoveryDeps, staleTasks: Task[]): void {
