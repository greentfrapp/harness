...
describe('TaskQueue', () => {
  ...
  describe('isDependencySatisfied', () => {
    ...
    it('returns false when dependency is done:null (read-only completion)', () => {
      const task = makeTask({ depends_on: 'dep-1' })
      deps.getTaskById.mockReturnValue(
        makeTask({ id: 'dep-1', status: 'done', substatus: null }),
      )
    ...
    it('returns false when dependency is in_progress:running', () => {
      const task = makeTask({ depends_on: 'dep-1' })
      deps.getTaskById.mockReturnValue(
        makeTask({ id: 'dep-1', status: 'in_progress', substatus: 'running' }),
      )
      expect(queue.isDependencySatisfied(task)).toBe(false)
    })

    it('returns false when dependency is done:rejected', () => {
      const task = makeTask({ depends_on: 'dep-1' })
      deps.getTaskById.mockReturnValue(
        makeTask({ id: 'dep-1', status: 'done', substatus: 'rejected' }),
      )
      expect(queue.isDependencySatisfied(task)).toBe(false)
    })
  ...
  describe('dispatch', () => {
    it('transitions queued task to in_progress:running', () => {
      const task = makeTask({ id: 't1', status: 'queued' })
      const updated = makeTask({ id: 't1', status: 'in_progress', substatus: 'running' })
