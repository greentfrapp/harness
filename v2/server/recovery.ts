 ...
 * 1. Detect stale tasks (in_progress:running or in_progress:retrying)
...
export function recoverStaleTasks(deps: RecoveryDeps): number {
  // v2: stale tasks are in_progress with running or retrying substatus
  const staleTasks = deps
    .getTasksByStatus(['in_progress'])
    .filter(
      (t) =>
        t.substatus === 'running' ||
        t.substatus === 'retrying' ||
        t.substatus === 'waiting_on_subtasks',
    )
...
function recoverTask(deps: RecoveryDeps, task: Task): void {
  ...
  if (!project) {
    ...
    deps.updateTask(task.id, {
      ...
      result: 'Server restarted; project not found',
    })
  ...
  if (task.worktree_path && task.branch_name) {
    ...
    if (
      ...
      // Has partial work — push to inbox as pending:review so user can review
      const target = transition(
        task.status,
        task.substatus,
        'recover_error',
