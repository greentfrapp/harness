...
function makeDeps() {
  const pool = {
    ...
    dispatchDoTask: vi.fn().mockImplementation(async () => {
      pool.activeWorktreeCount++
    }),
    dispatchDiscussTask: vi.fn().mockImplementation(async () => {
      pool.activeConversationCount++
    }),
  }
  ...
  return {
    ...
    pool: pool as any,
    getProjectById: vi
      .fn<(id: string) => Project | undefined>()
...
describe('Dispatcher', () => {
  ...
  it('dispatches a queued Do task when worktree slot is available', async () => {
    ...
    expect(deps.updateTask).toHaveBeenCalledWith(task.id, {
      status: 'in_progress',
      substatus: 'running',
  ...
  it('marks task as pending:error when dispatch fails', async () => {
    ...
    // After dispatch transition, getTaskById returns the dispatched version
    deps.getTaskById
      .mockReturnValueOnce(task) // first call: for dispatch
      .mockReturnValue(dispatched) // subsequent calls: after status update
    deps.pool.dispatchDoTask.mockRejectedValueOnce(new Error('spawn failed'))

    await dispatcher.tryDispatch()

    // v2: dispatch_error transitions in_progress:running → pending:review
