...
import { ensureHarnessDir, loadConfig, validateConfig } from './config'
import type { AppContext, CheckoutEntry } from './context'
import { initDatabase } from './db/index'
import * as queries from './db/queries'
import { Dispatcher } from './dispatcher'
import {
  cleanupCheckoutBranches,
  getCurrentBranch,
  returnCheckout,
} from './git'
...
console.log('Harness v2 starting...')
...
// eslint-disable-next-line prefer-const -- reassigned after pool creation to break circular dep
...
const pool = new AgentPool({
  ...
  broadcast: (event, data) => sseManager.broadcast(event, data),
  getTaskById: queries.getTaskById,
  getTaskProposals: queries.getTaskProposals,
  onTaskCompleted: (taskId: string) => {
    // Re-check queue when an agent finishes (slot freed)
    dispatcher?.tryDispatch()
    // checkWaitingParent is handled in routes/tasks.ts (approve, reject, cancel)
  },
})
...
for (const project of queries.getAllProjects()) {
  ...
  if (currentBranch && currentBranch.startsWith('harness/checkout-')) {
    ...
    if (matchedTask) {
      ...
      console.log(
        `Recovered checkout: ${currentBranch} → task ${matchedTask.id}`,
      )
    } else {
...
app.route('/api', createCheckoutRoutes(appContext))
app.route('/api', createProjectRoutes(appContext))
app.route('/api', createViewRoutes())

// Serve static files in production
app.use('/*', serveStatic({ root: './v2/client/dist' }))
app.get('/*', serveStatic({ root: './v2/client/dist', path: 'index.html' }))
...
serve({ fetch: app.fetch, port: PORT }, (info) => {
  console.log(`Harness v2 server listening on http://localhost:${info.port}`)
