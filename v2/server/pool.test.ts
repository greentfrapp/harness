...
// --- Plan approval / transition requests ---

describe('AgentPool plan approval', () => {
  it('intercepts ExitPlanMode and moves task to pending:review via request_transition', async () => {
    const updateTask = vi.fn(() => makeTask())
    const createTaskEvent = vi.fn()
    const onTaskCompleted = vi.fn()
    const broadcast = vi.fn()

    // Adapter that detects ExitPlanMode
    ...
    // v2: request_transition → pending:review (not held)
    expect(updateTask).toHaveBeenCalledWith(
      'task-1',
      expect.objectContaining({
        status: 'pending',
