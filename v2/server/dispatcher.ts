...
export class Dispatcher {
  ...
  private async dispatchTasks(opts: {
    ...
    while (opts.getActiveCount() < opts.getLimit()) {
      ...
      if (!project) {
        // v2: dispatch_error → pending:review
        const target = transition(task.status, task.substatus, 'dispatch_error')
      ...
      // v2: dispatch → in_progress:running
      const target = transition(task.status, task.substatus, 'dispatch')
      this.deps.updateTask(task.id, {
        status: target.status,
        substatus: target.substatus,
        started_at: Date.now(),
      ...
      try {
        ...
        const errTarget = transition(
          current?.status ?? 'in_progress',
          current?.substatus ?? 'running',
          'dispatch_error',
        )
        this.deps.updateTask(task.id, {
          status: errTarget.status,
          substatus: errTarget.substatus,
          result: `Failed to dispatch: ${msg}`,
        })
