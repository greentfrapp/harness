...
const drafts = computed(() =>
  viewTasks.value.filter((t) => t.status === 'draft'),
)
...
async function handleClear() {
  ...
  const clearable = statuses.filter((s) => s !== 'draft')
  if (clearable.length) {
        ...
        <div v-if="!draftsCollapsed" class="space-y-2 mt-1">
          <div
            ...
            <div class="px-4 py-3 flex items-start gap-3">
              <!-- Draft icon -->
              <span
                class="mt-1 w-4 h-4 shrink-0 flex items-center justify-center">
                <svg
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1">
                  ...
                  <span
                    class="text-xs font-medium px-1.5 py-0.5 rounded bg-zinc-800/60 text-zinc-500 italic">
                    draft
                  </span>
                  <span
