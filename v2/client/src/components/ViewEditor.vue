...
import type { Priority, Project, TagConfig, TaskStatus, TaskSubstatus, ViewConfig } from '@shared/types'
...
const ALL_SUBSTATUSES: (TaskSubstatus & string)[] = [
  'running',
  'retrying',
  'waiting_on_subtasks',
  'review',
...
const selectedSubstatuses = ref<Set<string>>(
  new Set((props.view?.filter.substatuses ?? []).filter((s) => s !== null) as string[]),
)
...
function toggleSubstatus(substatus: string) {
  if (selectedSubstatuses.value.has(substatus)) {
    selectedSubstatuses.value.delete(substatus)
  } else {
    selectedSubstatuses.value.add(substatus)
  }
  selectedSubstatuses.value = new Set(selectedSubstatuses.value)
}
  ...
  const view: ViewConfig = {
    ...
    filter: {
      ...(selectedStatuses.value.size > 0
        ...
        : {}),
      ...(selectedSubstatuses.value.size > 0
        ? { substatuses: [...selectedSubstatuses.value] as TaskSubstatus[] }
        : {}),
      ...
      <div
        ...
        <div class="flex-1 overflow-y-auto px-6 py-4 space-y-5">
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-1.5">
              <button
                v-for="substatus in ALL_SUBSTATUSES"
                :key="substatus"
                class="px-2.5 py-1 text-xs font-medium rounded transition-colors"
                :class="
                  selectedSubstatuses.has(substatus)
                    ? 'bg-zinc-600 text-zinc-100'
                    : 'bg-zinc-800 text-zinc-500 hover:text-zinc-300'
                "
                @click="toggleSubstatus(substatus)">
                {{ substatus }}
              </button>
            </div>
          </div>
