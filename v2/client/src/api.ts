...
export const api = {
  ...
  tasks: {
    ...
    bulkCancel: (ids: string[]) =>
      request<{ cancelled: string[]; skipped: string[] }>(
        '/api/tasks/cancel',
        json({ ids }),
      ),
    delete: (id: string) =>
      request<{ deleted: string }>(`/api/tasks/${id}`, {
        method: 'DELETE',
      }),
    ...
    dismiss: (id: string) =>
      request<Task>(`/api/tasks/${id}/dismiss`, { method: 'POST' }),
    fix: (
      id: string,
    ...
    transitions: (id: string) =>
      request<TaskTransition[]>(`/api/tasks/${id}/transitions`),
    progress: (id: string) =>
    ...
    getProposals: (id: string) =>
      request<TaskProposal[]>(`/api/tasks/${id}/proposals`),
    resolveProposals: (
      id: string,
      body: {
...
// Re-export TagConfig for use in api.config.get() return type
